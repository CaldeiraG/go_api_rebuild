...
const SqlGEN3 = `select count(id) as prod, MAX(LEFT(RTRIM(CodeCeHousing), 1))  as model
					from Gen3_LineB_LocalDB.dbo.FinishedPallets
					WHERE LineB2Good=1 and LineNumber=2 and DateDataSave >= @dataInicial and DateDataSave <= @dataFinal`

const SqlInv3 = `select COUNT(MINDEX) as prod, MAX(LEFT(RIGHT(RTRIM(PART_NUMBER), 3),1)) as model
...
const SqlYF = `select COUNT(MINDEX) as prod, MAX(RIGHT(LEFT(RTRIM(END_ITEM_PART_NUMBER), 9),4)) as model
				 FROM [YF_Clone].[dbo].[LineD]
				 where REJECTED = 0 and WRITE_STATION = 990 and TIME_STAMP >= @dataInicial and TIME_STAMP <= @dataFinal`

const SqlInv4 = `select COUNT(MINDEX) as prod, MAX(RIGHT(LEFT(RTRIM(PN_INVERTER), 8),3)) as model
...
const SqlInv42 = `select COUNT(MINDEX) as prod, MAX(RIGHT(LEFT(RTRIM(PN_INVERTER), 8),3)) as model
				 FROM [GEN4_Inverter].[dbo].[INVERTER_LINE2_PROD]
				 where (REJECTED IS NULL OR REJECTED = 0) and WRITE_STATION = '90' and TIME_STAMP >= @dataInicial and TIME_STAMP <= @dataFinal`

const SqlInv43 = `select COUNT(MINDEX) as prod, MAX(RIGHT(LEFT(RTRIM(PN_INVERTER), 8),3)) as model
				 FROM [GEN4_Inverter].[dbo].[INVERTER_LINE3_PROD]
				 where (REJECTED IS NULL OR REJECTED = 0) and WRITE_STATION = '90' and TIME_STAMP >= @dataInicial and TIME_STAMP <= @dataFinal`

const SqlmodelCheck = `SELECT [name]
